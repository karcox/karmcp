...
/**
 * Handles the elementor-mcp → karmcp rename: detecting the still-active
 * legacy plugin and snapshotting its persisted settings into the new keys.
 *
 * @since 2.1.0
 */
class KarMCP_Migration {
	 ...
	 *
	 * @since 2.1.0 (since 2.0.0 as karmcp_legacy_plugin_active)
	 *
	 * @return bool
	 */
	 ...
	 * `karmcp_*` keys. Each option is seeded ONLY when the new key has never
	 ...
	 *
	 * @since 2.1.0 (since 2.0.0 as karmcp_migrate_legacy_data)
	 */
	public static function migrate(): void {
		$option_map = array(
			'elementor_mcp_disabled_tools'   => 'karmcp_disabled_tools',
			'elementor_mcp_defaults_applied' => 'karmcp_defaults_applied',
			'elementor_mcp_server_enabled'   => 'karmcp_server_enabled',
		);
		$sentinel = '__karmcp_missing__';
		...
		if ( ! get_option( 'karmcp_legacy_meta_migrated' ) ) {
			global $wpdb;
			$meta_map = array(
				'elementor_mcp_upgrade_notice_dismissed'   => 'karmcp_upgrade_notice_dismissed',
				'elementor_mcp_community_notice_dismissed' => 'karmcp_community_notice_dismissed',
			);
			...
			update_option( 'karmcp_legacy_meta_migrated', 1 );
		}
	 ...
	 * namespace (`elementor-mcp/…`) to the new one (`karmcp/…`), introduced in
	 * v3.0.0 alongside the broader move beyond Elementor.
	 *
	 * The `karmcp_disabled_tools` option stores ability NAMES, which changed
	...
	private static function migrate_ability_slug_namespace(): void {
		if ( get_option( 'karmcp_slug_namespace_migrated' ) ) {
			return;
		}
		...
		if ( is_array( $disabled ) && ! empty( $disabled ) ) {
			...
			foreach ( $disabled as $slug ) {
				...
				if ( 0 === strpos( $slug, 'elementor-mcp/' ) ) {
					$slug = 'karmcp/' . substr( $slug, strlen( 'elementor-mcp/' ) );
				}
				$remapped[] = $slug;
			}
			$remapped = array_values( array_unique( $remapped ) );
			if ( $remapped !== $disabled ) {
				update_option( 'karmcp_disabled_tools', $remapped );
			}
		}

		update_option( 'karmcp_slug_namespace_migrated', 1 );
	}
}
