...
| Versión actual | `1.43.2` — en `karmcp.php` (cabecera + `KARMCP_VERSION`) y `readme.txt` (`Stable tag`); los tres tienen que coincidir, y `VersionTripleTest` lo comprueba |
...
Estado de referencia (medido el 2026-09-23): **1.579 tests, 10.093 aserciones**; **PHPStan sin errores**; **PHPCS sin errores ni avisos**. Las tres bloquean. Cualquier hallazgo que veas lo ha introducido lo que estés cambiando.
...
- **Sí hay updater desde la 1.43.0, y es propio.** La cabecera lleva `Update URI: https://github.com/karcox/karmcp`, que es lo que hace a WordPress preguntar por el filtro `update_plugins_github.com` en vez de buscar el slug en wordpress.org. Lo responde `KarMCP_Updater` leyendo la última release publicada del repo (público), con caché de 6 h y de 30 min en los fallos. Lo que devuelve es **la URL que WordPress descarga y descomprime sobre el plugin vivo**, así que exige tag con forma de versión, mayor que la instalada, y un `.zip` servido por github.com u objects.githubusercontent.com; cualquier otra cosa es "no hay actualización", nunca una conjetura. Apagado con `KARMCP_NO_UPDATE_CHECK` o con el filtro `karmcp_update_check_enabled`.
