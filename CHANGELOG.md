...
- **Fixed: the remote-connection config the admin generates.** It referenced an npm package that was never published, so every generated `npx` config failed to start. It now points at `karmcp-proxy`, and the proxy's own documentation matches the environment variables the proxy actually reads (`KARMCP_SITES`, `KARMCP_SITES_FILE`, `KARMCP_DEFAULT_SITE`).
