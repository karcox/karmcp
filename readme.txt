...
* **Plugins & themes**: discover, install, update, activate and delete. Installs come from wordpress.org, or — once you enable the tool — from a ZIP uploaded to the Media Library, checked against its SHA-256 and inspected before extraction. KarMCP and Elementor are protected from deactivation.
...
= 1.37.1 =
Fixed: OAuth sign-in dead-ended for anyone who was not already logged into WordPress. The authorize endpoint rebuilt its login-return URL with sanitize_text_field(), which strips percent-encoded characters, so a client's `redirect_uri=http%3A%2F%2F127.0.0.1%3A33418%2Fcallback` came back from wp-login.php as `http127.0.0.133418callback` - no longer the URI the client registered, so authorize refused it and the client never received a code. Connecting from a browser that already held a session skipped the login redirect and always worked, which is why it went unnoticed.
...
* Added: a written performance audit of the plugin itself, published in the source repository: what each kind of request costs, how it was measured, what is worth fixing next, and which obvious suspects turned out to be harmless.
...
* Changed: file search now accepts a single file as well as a directory, instead of answering "Not a directory." — and when a path is wrong it explains what it accepts.
