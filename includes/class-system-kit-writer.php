 ...
 * @since 1.8.0
 */
class KarMCP_System_Kit_Writer {
	...
	public static function apply_kit( array $kit ) {
		if ( empty( $kit['colors'] ) || ! is_array( $kit['colors'] ) || empty( $kit['typography'] ) || ! is_array( $kit['typography'] ) ) {
			return new WP_Error( 'invalid_kit', __( 'This brand kit is missing its colors or typography data.', 'karmcp' ) );
		}
	...
	public static function replace_system_colors( array $colors ) {
		if ( ! self::user_has_access() ) {
			return new WP_Error( 'insufficient_capability', __( 'You need the manage_options capability to apply brand kits.', 'karmcp' ) );
		}
		...
		foreach ( self::SYSTEM_SLOTS as $slot ) {
			if ( ! isset( $colors[ $slot ] ) || ! is_array( $colors[ $slot ] ) ) {
				return new WP_Error(
					...
					sprintf( __( 'Brand kit colors must include all four system slots. Missing: %s.', 'karmcp' ), $slot )
				);
			}
			...
			if ( empty( $hex ) ) {
				return new WP_Error(
					...
					sprintf( __( 'The "%s" slot has an invalid hex color. Aborting, a brand kit replaces the whole palette or none of it.', 'karmcp' ), $slot )
				);
			}
		...
		if ( ! self::persist( $kit, array( 'system_colors' => $entries ) ) ) {
			return new WP_Error( 'persist_failed', __( 'Could not persist the new system colors to the Elementor kit.', 'karmcp' ) );
		}
	...
	public static function replace_system_typography( array $typography ) {
		...
		foreach ( self::SYSTEM_SLOTS as $slot ) {
			if ( ! isset( $typography[ $slot ] ) || ! is_array( $typography[ $slot ] ) ) {
				return new WP_Error(
					...
					sprintf( __( 'Brand kit typography must include all four system slots. Missing: %s.', 'karmcp' ), $slot )
				);
			}
		...
		if ( ! self::persist( $kit, array( 'system_typography' => $entries ) ) ) {
			return new WP_Error( 'persist_failed', __( 'Could not persist the new system typography to the Elementor kit.', 'karmcp' ) );
		}
	...
	public static function replace_brand_custom_colors( array $custom_colors ) {
		...
		foreach ( $custom_colors as $color ) {
			...
			$kept[] = array(
				...
				'title' => '' !== $title ? $title : __( 'Brand Color', 'karmcp' ),
				'color' => $hex,
			);
			$added++;
		}
		...
		if ( ! self::persist( $kit, array( 'custom_colors' => array_values( $kept ) ) ) ) {
			return new WP_Error( 'persist_failed', __( 'Could not persist the brand custom colors to the Elementor kit.', 'karmcp' ) );
		}
	...
	public static function apply_theme_style( array $colors, array $typography ) {
		...
		if ( '' === $body_family || '' === $heading_family || empty( $text_color ) ) {
			return new WP_Error( 'incomplete_theme_style', __( 'Brand kit is missing the fonts or text color needed to set theme defaults.', 'karmcp' ) );
		}
		...
		if ( ! self::persist( $kit, $settings ) ) {
			return new WP_Error( 'persist_failed', __( 'Could not persist the theme style defaults to the Elementor kit.', 'karmcp' ) );
		}
	...
	public static function restore_snapshot( array $snapshot, bool $full_clobber = false ) {
		if ( ! self::user_has_access() ) {
			return new WP_Error( 'insufficient_capability', __( 'You need the manage_options capability to restore brand kits.', 'karmcp' ) );
		}
		...
		if ( empty( $write ) ) {
			return new WP_Error( 'empty_snapshot', __( 'The backup did not contain any global settings to restore.', 'karmcp' ) );
		}

		if ( ! self::persist( $kit, $write ) ) {
			return new WP_Error( 'persist_failed', __( 'Could not persist the restored global settings to the Elementor kit.', 'karmcp' ) );
		}
	...
	private static function get_kit() {
		if ( ! class_exists( '\Elementor\Plugin' ) || ! isset( \Elementor\Plugin::$instance->kits_manager ) ) {
			return new WP_Error( 'no_elementor', __( 'Elementor is not available.', 'karmcp' ) );
		}
