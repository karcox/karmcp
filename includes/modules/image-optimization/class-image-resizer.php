 ...
 * original under `uploads/karmcp-originals/` (reversible), then regenerates all
 * sub-sizes + metadata via wp_generate_attachment_metadata() — which re-runs the
 * module's own compress + WebP pipeline (the `_karmcp_optim` marker is cleared first
...
class KarMCP_Image_Resizer {
	...
	public static function validate_input( string $mime, $width, $height, bool $crop ) {
		if ( ! in_array( $mime, self::RESIZABLE, true ) ) {
			return new WP_Error( 'not_image', __( 'That attachment is not a resizable image (JPEG, PNG, WebP or GIF).', 'karmcp' ) );
		}
		$w = ( null !== $width && (int) $width > 0 ) ? (int) $width : null;
		$h = ( null !== $height && (int) $height > 0 ) ? (int) $height : null;
		if ( null === $w && null === $h ) {
			return new WP_Error( 'no_dimensions', __( 'Provide a positive width and/or height.', 'karmcp' ) );
		}
		if ( $crop && ( null === $w || null === $h ) ) {
			return new WP_Error( 'crop_needs_both', __( 'Cropping requires both a width and a height.', 'karmcp' ) );
		}
		return true;
	}
	...
	public static function resize( int $attachment_id, $width, $height, bool $crop = false ) {
		...
		if ( ! $post || 'attachment' !== $post->post_type ) {
			return new WP_Error( 'not_found', __( 'Attachment not found.', 'karmcp' ) );
		}
		...
		if ( ! $file || ! file_exists( $file ) ) {
			return new WP_Error( 'file_missing', __( 'The attachment file could not be found on disk.', 'karmcp' ) );
		}
		...
		$backup = KarMCP_Image_Optimizer::backup_path( $file, $upload );
		...
		$editor->set_quality( KarMCP_Image_Optimizer::clamp_quality( (int) get_option( self::quality_option(), 60 ) ) );
		...
		delete_post_meta( $attachment_id, KarMCP_Image_Optimizer::META_KEY );
	...
	private static function quality_option(): string {
		return KarMCP_Image_Optimization_Module::PREFIX . 'quality';
	}
}
